...
class Provider {
  private useProxyBypass = "{{useProxyBypassSushiScan}}";
  private proxyBypassUrl = "{{proxyBypassUrlSushiScan}}";
  private baseUrl = "https://sushiscan.net";
  private cookies = "";
  private userAgent = "";
  ...
  async search(opts: QueryOptions): Promise<SearchResult[]> {
    ...
    const series: SearchResult[] = data.series[0].all.map((item) => ({
    ...
    for (const serie of series) {
      const getExtraData = await this.safeFetch(`${this.baseUrl}${serie.id}`);
      if (getExtraData.ok) {
        const html = await getExtraData.text();

        const $ = LoadDoc(html);
        $(".infotable tr").each((i, row) => {
          const rowText = row.text()?.trim() ?? "";
          if (rowText.includes("Année")) {
            const year = rowText.split("Année")[1].trim();
            serie.year = year ? parseInt(year) : undefined;
          }
        });
      }
    }
    ...
    if (opts.year) {
      return series.filter((serie) => serie.year === opts.year);
    } else {
      return series;
    }
  }
  ...
  private async getValidSessionHeaders(): Promise<void> {
    try {
      const res = await fetch(`${this.proxyBypassUrl}/v1`, {
        ...
        body: JSON.stringify({
          cmd: "request.get",
          url: `${this.baseUrl}/wp-content/uploads/cropped-IconSushiScan-192x192.png`,
          maxTimeout: 60000,
        }),
      });
